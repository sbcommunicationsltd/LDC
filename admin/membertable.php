...
{
	$appid = $_GET['approved'];
	$query2 = "SELECT * FROM Members WHERE ID = $appid";
	...
	if(mail($to, $subject, $message, $headers))
	{
		...
		$expire = date('Y-m-d H:i', strtotime(date("Y-m-d H:i", strtotime($joined)) . " +12 months"));
		
		$query3 = "UPDATE Members SET Approved = 'Yes', DateExpire = '$expire' WHERE ID = '$appid'";
		$result3 = mysql_query($query3) or die(mysql_error());?>

		<script>
		alert("Thank You! Member - <?php echo $fullname;?> has been approved!");
		parent.location.href='membershipdatabase.php?type=Silver';
		</script>
	<?php
		exit;
	} else {?>
...
if(isset($_GET['renew']))
{
	...
	$to = $email;
	//$to = 'sumita.biswas@gmail.com';
	$subject = 'Renewed Silver Membership to London Dinner Club';
	$body = "<html><head><title>Renewed Silver Membership to London Dinner Club</title></head><body>";
	$body .= "<p>Dear Silver Member,</p>";
	$body .= "<p>Thank you for renewing your Silver Membership for a further 6 months.</p>";
	...
	{*/
		...
		<script>
		alert("Thank You! Silver Member - <?php echo $fullname;?> has been renewed!");
		</script>
	<?php
		exit;
	/*}
...
<tr style="background-color:#999999; font-size:12px;">
	<th>
		<form method="post" name="sortedthird" action='membertable.php'>
	...
	while($row = mysql_fetch_array($result))
	{
		...
		<tr class='table' id='dataTable' 
			...
			foreach($fieldname as $field) {
				...
				if ($field!='ID' && $field!='Approved' && $field!='Image_Path') {?>
					<td <?php if($field=='Height' || $field=='Profession' || $field=='DietaryReq' || $field=='Interests' || $field=='DOB'){echo "nowrap='nowrap'";}?>><?php echo $row[$field];?></td>
		<?php	}
			} ?>
		</tr>
  <?php	
